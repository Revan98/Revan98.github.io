...
async function importStateDb(file) {
  ...
  if (!result.length) {
    db.close();
    showToast("This database does not contain planner backup data.");
    return;
  }
...
exportDbBtn.addEventListener("click", () => {
  exportStateDb().catch((error) => {
    console.error(error);
    showToast("Could not export the backup database.");
  });
});
...
importDbInput.addEventListener("change", () => {
  ...
  importStateDb(file).catch((error) => {
    console.error(error);
    showToast("Could not load this backup database.");
  });
});
...
exportPngBtn.addEventListener("click", () => {
  exportCurrentStagePng().catch((error) => {
    console.error(error);
    showToast("Could not export this stage as PNG.");
  });
});
